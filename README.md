...
Сборки не подписаны. macOS при первом запуске ругается на неизвестного
разработчика: открывать через правую кнопку → «Открыть». Windows покажет
...
- GitHub Actions: `.github/workflows/release.yml` собирает `.dmg` на macOS и
  `.exe` на Windows, артефакты висят в запуске.
